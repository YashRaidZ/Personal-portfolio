            ...
            <div className="flex flex-wrap gap-3">
              {category.items.map((item, i) => (
                <motion.span
                  ...
                  tabIndex={0}
                  title={item.name}
                  className="cursor-default rounded-lg border border-white/10 bg-bg-elevated/60 px-4 py-2.5 font-mono text-sm text-text-light transition-shadow hover:border-accent-primary/40 hover:shadow-[0_0_18px_rgba(0,230,118,0.15)] focus-visible:border-accent-primary/40"
                >
                  {item.name}
                </motion.span>
              ))}
